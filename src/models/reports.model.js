...
Reports.getEmployeeAvgReport = (req, result) => {
    ltmDbConn.query("SELECT ID, Employee, Type, COUNT(*) AS EmployeeCount, IF(CAST((SUM(TIME_TO_SEC(CONCAT(SUBSTRING_INDEX(Duration, 'h', 1), ':', SUBSTRING_INDEX(SUBSTRING_INDEX(Duration, 'm', 1), 'h ', -1), ':', SUBSTRING_INDEX(Duration, 's', 1)))) / COUNT(*)) / 3600 AS DECIMAL(19,2)) < 1, CONCAT(CAST((SUM(TIME_TO_SEC(CONCAT(SUBSTRING_INDEX(Duration, 'h', 1), ':', SUBSTRING_INDEX(SUBSTRING_INDEX(Duration, 'm', 1), 'h ', -1), ':', SUBSTRING_INDEX(Duration, 's', 1)))) / COUNT(*)) / 60 AS DECIMAL(19,2)), ' Min'), CONCAT(ROUND((SUM(TIME_TO_SEC(CONCAT(SUBSTRING_INDEX(Duration, 'h', 1), ':', SUBSTRING_INDEX(SUBSTRING_INDEX(Duration, 'm', 1), 'h ', -1), ':', SUBSTRING_INDEX(Duration, 's', 1)))) / COUNT(*)) / 3600, 2), ' Hours')) AS AvgTimePerTicket, SUM(1) AS TotalTickets, MIN(STR_TO_DATE(StartTime, '%a %b %d %Y %H:%i:%s')) AS MinStartTime, MAX(STR_TO_DATE(EndTime, '%a %b %d %Y %H:%i:%s')) AS MaxEndTime, (SELECT COUNT(*) FROM legendtime.tbltime WHERE STR_TO_DATE(EndTime, '%a %b %d %Y %H:%i:%s') BETWEEN STR_TO_DATE(?, '%a %b %d %Y %H:%i:%s') AND STR_TO_DATE(?, '%a %b %d %Y %H:%i:%s') AND EndTime IS NOT NULL) AS TotalAllEmpTickets FROM legendtime.tbltime WHERE STR_TO_DATE(EndTime, '%a %b %d %Y %H:%i:%s') BETWEEN STR_TO_DATE(?, '%a %b %d %Y %H:%i:%s') AND STR_TO_DATE(?, '%a %b %d %Y %H:%i:%s') AND EndTime IS NOT NULL GROUP BY Employee ORDER BY TotalTickets DESC", [req.params.starttime, req.params.endtime, req.params.starttime, req.params.endtime], (err, res) => {
        if (err) {
