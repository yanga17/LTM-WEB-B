...
Reports.getClientHistoryReport = (req, result) => {
    ltmDbConn.query("SELECT t.ID, t.Customer, t.Activity, t.Support_No, t.StartTime, t.EndTime, t.Duration, t.Comments, t.Phone_Number, t.Solution, t.IssueType, t.Phone_Number, t.Support_No, t.Employee, c.Surname, t.Type, t.name FROM legendtime.tbltime t JOIN tbltechnicians c ON t.Employee = c.Technician WHERE STR_TO_DATE(t.StartTime, '%a %b %d %Y %H:%i:%s') BETWEEN STR_TO_DATE(?, '%a %b %d %Y %H:%i:%s') AND STR_TO_DATE(?, '%a %b %d %Y %H:%i:%s') ORDER BY t.Employee, t.StartTime DESC", [req.params.starttime, req.params.endtime], (err, res) => {
        if (err) {
...
Reports.getEmployeeTaskReport = (req, result) => {
    ltmDbConn.query("SELECT t.ID, t.Employee, c.Surname AS Surname, t.Activity, COUNT(t.Activity) AS TaskCount FROM legendtime.tbltime t JOIN tbltechnicians c ON t.Employee = c.Technician WHERE STR_TO_DATE(t.StartTime, '%a %b %d %Y %H:%i:%s') BETWEEN STR_TO_DATE(?, '%a %b %d %Y %H:%i:%s') AND STR_TO_DATE(?, '%a %b %d %Y %H:%i:%s') AND t.IssueType = 'Task' GROUP BY t.Employee, t.Activity ORDER BY TaskCount DESC", [req.params.starttime, req.params.endtime], (err, res) => {
        if (err) {
