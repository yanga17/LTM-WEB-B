...
Reports.getEmployeeAvgReport = (req, result) => {
    ltmDbConn.query("SELECT t.ID, t.Employee, COUNT(*) AS EmployeeCount, GROUP_CONCAT(DISTINCT t.Activity ORDER BY t.Activity SEPARATOR ', ') AS Activities,IF(CAST((SUM(TIME_TO_SEC(CONCAT(SUBSTRING_INDEX(t.Duration, 'h', 1), ':', SUBSTRING_INDEX(SUBSTRING_INDEX(t.Duration, 'm', 1), 'h ', -1), ':', SUBSTRING_INDEX(t.Duration, 's', 1)))) / COUNT(*)) / 3600 AS DECIMAL(19,2)) < 1, CONCAT(CAST((SUM(TIME_TO_SEC(CONCAT(SUBSTRING_INDEX(t.Duration, 'h', 1), ':', SUBSTRING_INDEX(SUBSTRING_INDEX(t.Duration, 'm', 1), 'h ', -1), ':', SUBSTRING_INDEX(t.Duration, 's', 1)))) / COUNT(*)) / 60 AS DECIMAL(19,2)), ' Min'), CONCAT(ROUND((SUM(TIME_TO_SEC(CONCAT(SUBSTRING_INDEX(t.Duration, 'h', 1), ':', SUBSTRING_INDEX(SUBSTRING_INDEX(t.Duration, 'm', 1), 'h ', -1), ':', SUBSTRING_INDEX(t.Duration, 's', 1)))) / COUNT(*)) / 3600, 2), ' Hours')) AS AvgTimePerTicket, (SELECT COUNT(*) FROM legendtime.tbltime WHERE STR_TO_DATE(EndTime, '%a %b %d %Y %H:%i:%s') BETWEEN STR_TO_DATE(?, '%a %b %d %Y %H:%i:%s') AND STR_TO_DATE(?, '%a %b %d %Y %H:%i:%s') AND EndTime IS NOT NULL) AS TotalAllEmpTickets FROM legendtime.tbltime t WHERE STR_TO_DATE(t.EndTime, '%a %b %d %Y %H:%i:%s') BETWEEN STR_TO_DATE(?, '%a %b %d %Y %H:%i:%s') AND STR_TO_DATE(?, '%a %b %d %Y %H:%i:%s') AND t.EndTime IS NOT NULL GROUP BY t.Employee ORDER BY EmployeeCount DESC", [req.params.starttime, req.params.endtime, req.params.starttime, req.params.endtime], (err, res) => {
        if (err) {
